...
class ReadAsyncStream extends Readable {
  _fn: (stream: Readable) => Promise<void>
  _reading: boolean

  constructor (fn: (stream: Readable) => Promise<void>) {
    super({ objectMode: true })
    this._fn = fn
  }
...
class ReadSyncStream extends Readable {
  _fn: (stream: Readable) => void;

  constructor (fn: (stream: Readable) => void) {
    super({ objectMode: true })
    this._fn = fn
  }
...
class ThroughAsyncStream extends Transform {
  _fn: (chunk: any, stream: Transform) => Promise<void>;

  constructor (fn: (chunk: any, stream: Transform) => Promise<void>) {
    super({ objectMode: true })
    this._fn = fn
  }
...
class ThroughSyncStream extends Transform {
  _fn: (chunk: any, stream: Transform) => void;

  constructor (fn: (chunk: any, stream: Transform) => void) {
    super({ objectMode: true })
    this._fn = fn
  }
...
export function readAsync (fn: (stream: Readable) => Promise<void>): Readable {
...
export function throughAsync (fn: (chunk: any, stream: Transform) => Promise<void>): Transform {
  return new ThroughAsyncStream(fn)
}

export function throughSync (fn: (chunk: any, stream: Transform) => void): Transform {
